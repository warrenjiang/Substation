...
/************************************************
函数名称 ： CheckSDOAndContinue
功    能 ： 检查SDO通信状态
参    数 ： d 字典  nodeId：节点ID
返 回 值 ： 无
*************************************************/ 
...
/************************************************
函数名称 ： ConfigureSlaveNode
功    能 ： 配置节点参数，在收到节点上电信息时_post_SlaveBootup时调用一次，SDO通信一次 调用一次
参    数 ： d 字典  nodeId：节点ID
返 回 值 ： 无
*************************************************/ 
static void ConfigureSlaveNode(CO_Data* d, UNS8 nodeId)
{
	/*在这里可以配置节点参数，心跳，传输类型，对字典读写*/
	UNS8 Transmission_Type = 0xFE;//特定事件发生
	UNS16 heart_time=0x4E20; //心跳时间
	UNS8 res;
	MSG_WAR(0x0,"Master : ConfigureSlaveNode %2.2x\n", nodeId);

	switch(++init_step){
		case 1: /*配置节点TPDO 为事件发送*/
			MSG_WAR(0x0,"Master : set slave %2.2x TPDO 1 transmit type\n", nodeId);
			res = writeNetworkDictCallBack (d, /*CO_Data* d*/
					...
					CheckSDOAndContinue, /*回调函数*/
                   0); /* use block mode */
					break;	
			case 2: /*配置节点心跳时间*/
			MSG_WAR(0x0,"Master : set slave %2.2x heart_time\n", nodeId);
			res = writeNetworkDictCallBack (d, /*CO_Data* d*/
					nodeId, /*节点ID*/
					0x1017, /*UNS16 字典索引*/
					0x00, /*UNS8 字典子索引*/
...
{
	MSG_ERR(0x0,"..._heartbeatError.....:%x\r\n",heartbeatID);
}
void _post_SlaveBootup(CO_Data* d, UNS8 SlaveID)
{
	ConfigureSlaveNode(d,SlaveID);
}
