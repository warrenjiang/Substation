...
{   
	  uint8_t first[36]={0xFF,0x24,0x01,0x00,0x00,0xc0,0xA8,0x00,0x94,0x9C,0xA5,0x25,0x9A,0x90,0x1B,\
		...
		uint8_t mac[6]={0};
		char strip[20]={0};//ÉÏÎ»»úÏÂ·¢ip Îª×Ö·û´®¸ñÊ½
		uint32_t ipval=0;
		char password[41]={0};//(ÓÃ»§Ãû+¡¯,¡®+ÃÜÂë)³¤¶ÈÖ®ºÍ²»ÄÜ³¬¹ý40 
		char *pr=NULL;
		uint8_t i;
		struct pbuf *ptr;
	  uint8_t type=0;//ÏûÏ¢ÀàÐÍ
	  ...
	  switch(type)
		{
           ...
           //ptr->payload=(void*)first;	//»áÔì³É±ÀÀ££¿			 
           ...
           pbuf_free(ptr);					 
			...
			case 0x03:
			    ...
			    App_Printf("\r\n");
					/*ÅäÖÃ¹ý¾²Ì¬ip,0xc0 STATIC 0x40:DHCP*/
          if(sysCfg.parameter.dhcp==STATIC)
           ...
           pbuf_free(ptr);					 
				...
				sysCfg.parameter.config_hold_flag = 0xff;  /*»Ö¸´³ö³§ÉèÖÃ*/
			  sysCfg.parameter.dhcp = DHCP;			 /*Ä¬ÈÏÎª¿ªÆôDHCP*/
				if(0 == bsp_WriteCpuFlash(SYSCFG_ADDR,sysCfg.data,SYSCFG_DATA_LEN))
				{
					delay_ms(100);
					NVIC_SystemReset();		
...
void Task_TCP_Client(void *pvParameters)
{
  ...
  static  uint16_t wait_main_socket_ack_time = 0;
	...
	memcpy(sysCfg.parameter.gw,(char*)&netif.gw.addr,4); //»ñÈ¡Íø¿¨ÖÐµÄÍø¹Ø
	memcpy(sysCfg.parameter.sub,(char*)&netif.netmask.addr,4);//»ñÈ¡Íø¿¨ÖÐµÄÑÚÂë
	udp_pc_init();//³õÊ¼»¯UDP,ÓÃÓÚÓëÉÏÎ»»úÍ¨Ñ¶
	udppcb=udp_ntp_init();//³õÊ¼»¯UDP£¬ÓÃÓÚntpÍ¨ÐÅ
  app_tcp_init();//³õÊ¼»¯tcp
	while(1)
	{	
		/*¼ì²âÖ÷¶Ë¿ÚÁ¬½Ó×´Ì¬*/
